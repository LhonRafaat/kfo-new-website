...
import type { IndustryGuidePage } from "@/lib/strapi";
 ...
 * into the uploaded asset instead of stacking a gradient over it.
 *
 * The text block is 567px wide with the paragraph held to 521px, both left
 * aligned inside it, and 24px between them.
 */
export function IndustryGuideHero({ page }: { page: IndustryGuidePage }) {
  const image = media(page.heroImage);
      ...
      <div className="container-edge relative z-10">
        <div className="mx-auto flex max-w-[567px] flex-col gap-6">
          <Reveal as="h1" className="heading-section text-cream">
            {page.title}
            {page.titleEmphasis && (
              <em className="font-normal italic">{page.titleEmphasis}</em>
            )}
          </Reveal>
